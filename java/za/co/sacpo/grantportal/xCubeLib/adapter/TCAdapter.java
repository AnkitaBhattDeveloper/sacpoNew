...
public class TCAdapter extends RecyclerView.Adapter<TCAdapter.NGHolder> {
    ...
    boolean full;
    Bitmap bitmap;
    String imageUrl,getImage6;
    public TCAdapter(List<TCObj.Item > aObjList, Context baseActivityContext, AppCompatActivity activityInCall, String CAId) {
    ...
    public TCAdapter.NGHolder onCreateViewHolder(ViewGroup parent, int viewType) {

        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.t_comments_row, parent, false);
        return new TCAdapter.NGHolder(itemView);

    }

    @Override
    public void onBindViewHolder(final TCAdapter.NGHolder holder, final int position) {
        ...
        /*  if(aObjList.getImage6 != null && !c.getGetImage6.isEmpty())*/
        /*
        if (getImage6==null) {
=======
        if (aObjList.isEmpty()) {
>>>>>>> 4478d98e5ef9431f84ab67d52f16c3bfaa009fec
            holder.GetImage.setImageResource(R.drawable.place_holder);
        } else{
            Picasso.get().load(aObjList.get(holder.getAdapterPosition()).getGetImage6()).resize(120, 60).into(holder.GetImage);
